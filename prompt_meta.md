...
`krl_multimedia_dict_android_vNN.md` 
...
---
На основе актуального repository snapshot (в файле `krl_multimedia_dict_android_vNN.md`) и моих наблюдений выше: тщательно проанализируй и детально сообщи, какие ошибки есть в коде и как их можно исправить?
...
Сформулируй CLI prompt (на основе твоего предыдущего ответа и repository snapshot в файле `krl_multimedia_dict_android_vNN.md`) только для подзадачи <название выбранной подзадачи>.
...
CLI prompt для Qwen3-Coder-Next должен:
   ...
   - не упоминать repository snapshot и не советовать читать `krl_multimedia_dict_android_vNN.md`;
   - всегда указывать полный относительный путь при упоминании файла, например `lib/game_page.dart`, а не `game_page.dart`;
   - включать фрагменты кода только для критической логики, опасных асинхронных мест, сложной структуры данных или layout, который легко реализовать неверно;
   - указывать краткий ожидаемый итог подзадачи в наблюдаемой форме;
   - требовать тест для исправления, если проблема относится к воспроизводимой логике, данным, сохранению состояния или стабильному UI-сценарию;
   ...
   - соблюдать жизненный цикл Flutter и не выполнять side effect при построении интерфейса;
   - после `await` проверять жизненный цикл, когда далее используются widget state, `BuildContext`, `Navigator` или `ScaffoldMessenger`;
   - не менять несвязанные файлы, зависимости, конфигурацию сборки или UI без явного объяснения необходимости;
